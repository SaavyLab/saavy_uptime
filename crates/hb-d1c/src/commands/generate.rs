...
pub fn run(conn: &Connection, config: &D1CConfig) -> Result<()> {
    ...
    for (file_name, file_queries) in queries_by_file {
        if file_name == "mod" {
            anyhow::bail!("SQL file cannot be named 'mod.sql' as 'mod' is a reserved keyword in Rust");
        }
        if file_name == root_module_stem {
             anyhow::bail!("SQL file cannot be named '{}.sql' as it conflicts with the root module name", file_name);
        }
        ...
        fs::write(
            submodule_dir.join(format!("{}.rs", file_name)),
            formatted,
        )?;
    ...
    fs::write(Path::new(&config.out_dir).join(root_file_name), root_content)?;
