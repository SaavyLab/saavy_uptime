...
pub fn render_module(queries: &[Query], instrument: bool) -> TokenStream {
    let blocks = queries.iter().map(|q| render_query_block(q, instrument));
    quote! {
...
fn render_query_block(query: &Query, instrument: bool) -> TokenStream {
...
fn render_function(query: &Query, instrument: bool) -> TokenStream {
