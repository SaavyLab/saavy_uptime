...
pub async fn get_monitor_by_id(
    ...
    let stmt = d1
        .prepare(
            "SELECT id, org_id, name, kind, enabled, config_json, status, last_checked_at, last_failed_at, first_checked_at, rt_ms, region, last_error, next_run_at, created_at, updated_at FROM monitors WHERE id = ?1 AND org_id = ?2",
        );
...
pub async fn get_monitors_by_org_id(
    ...
    let stmt = d1
        .prepare(
            "SELECT id, org_id, name, kind, enabled, config_json, status, last_checked_at, last_failed_at, first_checked_at, rt_ms, region, last_error, next_run_at, created_at, updated_at FROM monitors WHERE org_id = ?1 ORDER BY created_at DESC",
        );
...
pub async fn list_due_monitors(
    ...
    let stmt = d1
        .prepare(
            "SELECT id, kind, config_json, status, first_checked_at, last_failed_at, next_run_at FROM monitors WHERE org_id = ?1 AND enabled = 1 AND (?2 IS NULL OR next_run_at IS NULL OR next_run_at <= ?2) ORDER BY COALESCE(next_run_at, 0) ASC LIMIT ?3",
        );
    let stmt = stmt
        .bind(
            &[
